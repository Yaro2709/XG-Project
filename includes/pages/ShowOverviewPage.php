	...
	switch ($_GET['mode'])
	{
		case 'renameplanet':
			...
			if ($_POST['action'] == $lang['ov_planet_rename_action'])
			{
				...
				if (preg_match("/[^A-zА-я0-9_\- ]/", $newname) == 1)
				{
		...
		default:
			...
			// Последние 5 ответов из чата START
            ...
            $msgtimestamp=date("m/d H:i:s", $result['timestamp']);
                $msg .= "<tr align='left'> <td width='110'><font color='PaleGreen'><b>[".$msgtimestamp."]</b></font></td> <td align='left'>".$usr.":</td>
                        <td align='left' width='400'>".$str."</td></tr>";
            }
            $msg .= '</table>';
            include("includes/msg_replace.php");
            $parse['LastChat'] = $msg;
			// Последние 5 ответов из чата END
