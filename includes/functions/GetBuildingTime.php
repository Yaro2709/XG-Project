...
if(!defined('INSIDE')){ die(header("location:../../"));}
	...
	function GetBuildingTime ($user, $planet, $Element)
	{
		...
		if       (in_array($Element, $reslist['build']))
		{
			...
			$time         = ((($cost_crystal) + ($cost_metal) + ($cost_deuterium)) / $game_config['game_speed']) * (1 / ($planet[$resource['14']] + 1)) * pow(0.5, $planet[$resource['15']]);
		...
		elseif (in_array($Element, $reslist['tech']))
		{
			...
			$time         = (($cost_metal + $cost_crystal + $cost_deuterium) / $game_config['game_speed']) / (($lablevel + 1) * 2);
			$time         = floor(($time * 60 * 60) * (1 - (($user['rpg_scientifique']) * SCIENTIFIQUE)));
		}
		elseif (in_array($Element, $reslist['defense']))
		{
			$time         = (($pricelist[$Element]['metal'] + $pricelist[$Element]['crystal'] + $pricelist[$Element]['deuterium']) / $game_config['game_speed']) * (1 / ($planet[$resource['21']] + 1)) * pow(1 / 2, $planet[$resource['15']]);
